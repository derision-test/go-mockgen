...
func generateType(typ types.Type, importPath string, variadic bool) *jen.Statement {
	...
	case *types.Chan:
		if t.Dir() == types.RecvOnly {
			return generator.Compose(jen.Op("<-").Chan(), recur(t.Elem()))
		}

		if t.Dir() == types.SendOnly {
			return generator.Compose(jen.Chan().Op("<-"), recur(t.Elem()))
		}

		return generator.Compose(jen.Chan(), recur(t.Elem()))

	case *types.Interface:
		methods := []jen.Code{}
		for i := 0; i < t.NumMethods(); i++ {
			methods = append(methods, generator.Compose(jen.Id(t.Method(i).Name()), recur(t.Method(i).Type())))
		}

		return jen.Interface(methods...)

	case *types.Map:
		return generator.Compose(jen.Map(recur(t.Key())), recur(t.Elem()))

	case *types.Named:
		return generateQualifiedName(t, importPath)

	case *types.Pointer:
		return generator.Compose(jen.Op("*"), recur(t.Elem()))

	case *types.Signature:
		params := []jen.Code{}
		for i := 0; i < t.Params().Len(); i++ {
			params = append(params, generator.Compose(jen.Id(t.Params().At(i).Name()), recur(t.Params().At(i).Type())))
		}
	...
	case *types.Slice:
		return generator.Compose(getSliceTypePrefix(variadic), recur(t.Elem()))

	case *types.Struct:
		fields := []jen.Code{}
		for i := 0; i < t.NumFields(); i++ {
			fields = append(fields, generator.Compose(jen.Id(t.Field(i).Name()), recur(t.Field(i).Type())))
		}
